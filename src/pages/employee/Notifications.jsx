...
const Notifications = () => {
  ...
  const [filterType, setFilterType] = useState('all'); // all, unread, job_match, application_update, message
  ...
  const stats = {
    ...
    unread: unreadCount,
    jobMatches: notifications.filter(n => n.type === 'job_match').length,
    applicationUpdates: notifications.filter(n =>
      n.type === 'application_update' || n.type === 'shortlisted' || n.type === 'rejected' || n.type === 'interview'
    ).length,
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-secondary-600 text-lg">Stay updated on your job search</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        ...
        <div className="card bg-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-secondary-600 font-medium text-sm">Job Matches</p>
              <p className="text-3xl font-bold text-blue-700 mt-2">{stats.jobMatches}</p>
            </div>
        ...
        <div className="card bg-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-secondary-600 font-medium text-sm">Application Updates</p>
              <p className="text-3xl font-bold text-green-700 mt-2">{stats.applicationUpdates}</p>
            </div>
        ...
        <div className="card bg-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-secondary-600 font-medium text-sm">Messages</p>
              <p className="text-3xl font-bold text-orange-700 mt-2">{stats.messages}</p>
            </div>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        ...
        <button
          onClick={() => setFilterType('job_match')}
          className={`px-4 py-2 rounded-lg font-semibold transition ${
            filterType === 'job_match'
              ? 'bg-blue-600 text-white'
              : 'bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200'
          }`}
        >
          Job Matches ({stats.jobMatches})
        </button>
        <button
        ...
        >
          App Updates ({stats.applicationUpdates})
        </button>
        <button
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-grow">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className={`font-semibold ${notification.isRead ? 'text-secondary-900' : 'text-secondary-900 font-bold'}`}>
                            {notification.title}
                          </h3>
                          {!notification.isRead && (
                        ...
                        {notification.companyName && (
                          <p className="text-secondary-500 text-sm">{notification.companyName}</p>
                        )}
                        <p className="text-secondary-700 text-sm mt-2">{notification.description}</p>
                        <div className="flex items-center gap-4 mt-3">
                          <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                            notification.type === 'job_match' ? 'bg-blue-100 text-blue-700' :
                            notification.type === 'message' ? 'bg-orange-100 text-orange-700' :
                            notification.type === 'rejected' ? 'bg-red-100 text-red-700' :
